//변수 선언 및 초기화
var imgPath = '/home/siiru/se2/img/';
var nImageInfoCnt = 0;
...
//구분값
var viewTp = $Element("viewTp").attr("value");
var viewId = $Element("viewId").attr("value");
...
function goStartMode() {
	...
	if (sSrc.indexOf("btn_confirm2.png") < 0) {
		welBtnConfirm.attr("src",imgPath+"btn_confirm2.png");
...
function goReadyMode() {
	...
	if (sSrc.indexOf("btn_confirm2.png") >= 0) {
		...
		welBtnConfirm.attr("src",imgPath+"btn_confirm.png");
	}
}
/**
 * 일반 업로드
...
function addImage(ofile) {
	...
	if (!!bExceedLimitTotalSize) {
		...
		aFileList.push('<a onclick="delImage(\'img'+nImageInfoCnt+'\')"><img class="del_button" src="'+imgPath+'btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
		aFileList.push('</li> ');
...
function html5Upload() {
	var tempFile,
		sUploadURL;
	sUploadURL= '/editorimageUpload.do?fileTp=M&viewTp='+viewTp+'&viewId='+viewId; 	//upload URL
	//파일을 하나씩 보내고, 결과를 받음.
...
function callFileUploader() {
	oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
		sUrl  : location.href.replace(/\/[^\/]*$/, '') + '/editorimageUpload.do?fileTp=S&viewTp='+viewTp+'&viewId='+viewId,	//샘플 URL입니다.
		sCallback : location.href.replace(/\/[^\/]*$/, '') + '/editorImageCallback.do',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
